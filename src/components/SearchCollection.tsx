...
// Generic type that works with any collection
...
export default function SearchCollection<T extends string>({ entry_name, data, tags }: Props<T>) {
  const [query, setQuery] = createSignal("");
  const [filter, setFilter] = createSignal(new Set<string>())
  const [collection, setCollection] = createSignal<CollectionEntry<T>[]>([])
  const [descending, setDescending] = createSignal(false);

  // Fuse.js configuration with proper typing
  ...
  // Enhanced ArrowCard with service icon support
  const EnhancedCard = ({ entry }: { entry: CollectionEntry<T> }) => {
    const icon = entry.collection === 'services' && serviceIcons[entry.data.title.toLowerCase().replace(/\s+/g, '-') as keyof typeof serviceIcons];
    
    return (
      <ArrowCard entry={entry}>
        {icon && (
          <div class="absolute top-4 right-4 text-2xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white p-2 rounded-lg">
            {icon}
          </div>
        )}
      </ArrowCard>
